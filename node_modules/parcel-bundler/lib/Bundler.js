...
class Bundler extends EventEmitter {
  ...
  buildQueuedAssets(isInitialBundle = false) {
    ...
    return _asyncToGenerator(function* () {
      ...
      while (_this2.buildQueue.size > 0) {
        let promises = [];
        ...
        try {
          for (var _iterator = _this2.buildQueue[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            if (!isInitialBundle) {
              asset.invalidate();
              if (_this2.cache) {
                _this2.cache.invalidate(asset.name);
              }
            }

            promises.push(_this2.loadAsset(asset));
  ...
  resolveAsset(name, parent) {
    ...
    return _asyncToGenerator(function* () {
      var _ref = yield _this3.resolver.resolve(name, parent);

      let path = _ref.path,
          pkg = _ref.pkg;

      if (_this3.loadedAssets.has(path)) {
        return _this3.loadedAssets.get(path);
      }

      let asset = _this3.parser.getAsset(path, pkg, _this3.options);
      _this3.loadedAssets.set(path, asset);

      if (_this3.watcher) {
        _this3.watcher.add(path);
      }
  ...
  resolveDep(asset, dep) {
    ...
    return _asyncToGenerator(function* () {
      try {
        return yield _this4.resolveAsset(dep.name, asset.name);
      } catch (err) {
  ...
  loadAsset(asset) {
    ...
    return _asyncToGenerator(function* () {
      if (asset.processed) {
        _this5.buildQueue.delete(asset);
        return;
      }

      if (!_this5.errored) {
        _this5.logger.status('⏳', `Building ${asset.basename}...`);
      }
      ...
      let processed = _this5.cache && (yield _this5.cache.read(asset.name));
      if (!processed) {
        processed = yield _this5.farm.run(asset.name, asset.package, _this5.options);
        if (_this5.cache) {
          _this5.cache.write(asset.name, processed);
        }
      }
      ...
      if (_this5.delegate.getImplicitDependencies) {
        let implicitDeps = yield _this5.delegate.getImplicitDependencies(asset);
        if (implicitDeps) {
      ...
      yield Promise.all(dependencies.map((() => {
        var _ref2 = _asyncToGenerator(function* (dep) {
          let assetDep = yield _this5.resolveDep(asset, dep);
          if (dep.includedInParent) {
            // This dependency is already included in the parent's generated output,
            // so no need to load it. We map the name back to the parent asset so
            // that changing it triggers a recompile of the parent.
            _this5.loadedAssets.set(dep.name, asset);
          } else {
            asset.dependencies.set(dep.name, dep);
            asset.depAssets.set(dep.name, assetDep);
            yield _this5.loadAsset(assetDep);
          }
        });
  ...
  *findOrphanAssets() {
  ...
  onChange(path) {
    ...
    return _asyncToGenerator(function* () {
      let asset = _this6.loadedAssets.get(path);
      if (!asset) {
        return;
      }
      ...
      _this6.buildQueue.add(asset);
      clearTimeout(_this6.rebuildTimeout);

      _this6.rebuildTimeout = setTimeout(_asyncToGenerator(function* () {
        yield _this6.bundle();
      }), 100);
    })();
  }
